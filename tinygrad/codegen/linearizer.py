...
class Linearizer(Kernel):
  ...
  def render_reduceop(self, reduceop:LazyOp, seq: int, accs:Dict[LazyOp, List[UOp]],
    ...
    alias_buf_idxs = []
    for i in [i for i in self.local_alias if i in self.tensor_core_opts.bufs[self.reduceops.index(reduceop)]]:
      localbuf_idx = self.bufs.index(self.local_alias[i])
      buf_idxs = [idx*0 if s == 0 else idx for idx,s in zip(global_idxs+local_idxs+reduce_idxs+full_upcast_idxs,self.sts[i].real_strides())]
      if (tc:=self.tensor_core):
        min_alias_idx = [min(*pair) for pair in self.tensor_core_opts.bufs if i in pair][0]
  ...
  def linearize(self):
    ...
    if (tc:=self.tensor_core) and (tc_opts:=self.tensor_core_opts):
      ...
      for (tc_buf0,tc_buf1) in tc_opts.bufs:
        self.alias_buffer(tc_buf0, alias_pattern)
        self.alias_buffer(tc_buf1, alias_pattern)
