...
class TestLinearizer(unittest.TestCase):
  ...
  @unittest.skipUnless(Device[Device.DEFAULT].renderer.tensor_cores, "test requires tensor cores")
  def test_tensor_multireduce_tc_not_tc(self):
    N = 128
    Tensor.manual_seed(1552)
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      # bf16 buffer returns float32 numpy outputs so test would fail. testing opt with half suffices.
      ...
      ast = LazyOp(op=BufferOps.STORE, src=(
        LazyOp(op=ReduceOps.SUM, src=(
          LazyOp(op=BinaryOps.DIV, src=(
            LazyOp(op=BufferOps.LOAD, src=(), arg=MemBuffer(idx=3, dtype=dtypes.float32, st=ShapeTracker(views=(View(shape=(128,128,128),strides=(128,1,0),offset=0,mask=None,contiguous=False),)))),
            LazyOp(op=ReduceOps.SUM, src=(
